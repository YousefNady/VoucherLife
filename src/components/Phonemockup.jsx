  ...
  return (
    <div className="relative mx-auto w-64 sm:w-72">
        ...
        <div className="bg-gray-900 rounded-[2rem] overflow-hidden">
          ...
          <div className="bg-white mx-2 mb-2 rounded-2xl overflow-hidden">
            ...
            <div className="mx-3 mt-3 bg-brand-gradient rounded-xl p-4 text-white">
              <p className="text-xs font-medium opacity-80 mb-0.5">{p.exclusive}</p>
              <p className="text-sm font-bold leading-tight">{p.discounts}</p>
              ...
              <p className="text-xs opacity-70 mt-1">{p.validAt}</p>
            </div>

            <div className="px-3 py-3">
              <p className="text-xs font-semibold text-gray-500 mb-2.5">{p.popularCategories}</p>
              <div className="grid grid-cols-3 gap-2">
                {[
                  { icon: '🏥', label: p.categories.hospitals },
                  { icon: '🧪', label: p.categories.labs },
                  { icon: '💊', label: p.categories.pharmacy },
                  { icon: '🩻', label: p.categories.radiology },
                  { icon: '🩺', label: p.categories.clinics },
                  { icon: '📦', label: p.categories.packages },
                ].map(({ icon, label }) => (
      ...
      <div className="absolute -left-6 top-16 bg-white rounded-2xl shadow-xl px-3 py-2.5 flex items-center gap-2 border border-gray-100">
        ...
        <div>
          <div className="text-xs font-bold text-gray-800">{p.verified}</div>
          <div className="text-[10px] text-gray-500">{p.providers}</div>
        </div>
      </div>
      ...
      <div className="absolute -right-6 bottom-20 bg-white rounded-2xl shadow-xl px-3 py-2.5 flex items-center gap-2 border border-gray-100">
        ...
        <div>
          ...
          <div className="text-[10px] text-gray-500">{p.reviews}</div>
        </div>
      </div>
    </div>
  )
